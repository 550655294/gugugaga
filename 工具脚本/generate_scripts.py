...
| 6 | (v4.3) 24s连续脚本无需尾帧，操作卡不写🔚🔗尾帧项，提示词不用@图片2/ @image2作为首帧 | ☐ |
| ... | （继续按规范文档 §十三 列出全部检查项） | ☐ |
| 最后一列必须全部填 ✅ | ✅ |

### ✅ 24s脚本无需尾帧（v4.3新增铁律）
- 🚫 操作卡不要出现 🔚 尾帧截图 或 🔗 尾帧 行
- 🚫 提示词不要出现 @图片2作为首帧 或 @image2 as first frame
- ✅ 第二场景首段直接文字描述开场状态，自然承接第一场景结尾

### ✅ 中文提示词中必须写「⚠️ 角色铁律」（精确四个字）
规范要求每个场景标题下紧跟角色铁律。注意：必须写「⚠️ 角色铁律」，不是「⚠️ 铁律」。
缺少"角色"两个字会导致自动校验失败。"""
