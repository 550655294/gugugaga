...
# ═══ DeepSeek API ═══
def call_api(system_prompt, user_prompt, max_tokens=16000):
...
| 第一场景（0-12s） | 场景背景参考图 | [根据本集剧本填入具体场景描述，如：温馨厨房背景] |
| 第一场景（0-12s） | 是否需额外道具参考 | [如果有关键道具则列出，如：特定食物/物品；如果无则写"无需"] |
| 第二场景（12-24s） | 角色参考图 | 与第一场景同一张（保持角色一致） |
| 第二场景（12-24s） | 场景背景参考图 | [根据本集剧本填入，如：室外公园背景] |
| 第二场景（12-24s） | ⚠️ 是否需要第一场景尾帧 | 如果两场景之间画面需要连续/角色位置连贯 → "需要，作为首帧参考上传"；如果完全切换新场景 → "不需要，直接新场景生成" |
| 第二场景（12-24s） | 是否需额外道具参考 | [如果有则列出；无则写"无需"] |
| 全局 | 转场方式 | [说明两场景间如何过渡：直接切换/黑场过渡/特效转场/尾帧衔接] |
```
...
9. 素材需求清单必须根据本集剧本的具体内容填写，不能写占位符"""
...
def generate_one():
    ...
    try:
        ...
        user_prompt = f"请创作第{ep_num}集的完整分镜脚本（8段双章节24秒格式）。\n要求：新主题（不能是已有主题）、有笑点有反转、包含完整中英文提示词和合并提示词。直接输出完整Markdown。"
        
        response = call_api(sys_prompt, user_prompt, 16000)
        _add_log("✅ API响应完成")
