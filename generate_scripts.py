...
def analyze_usage_stats():
    ...
    char_counts = {"Doro": 0, "哈基蜂": 0, "小黄鸡": 0}
    char_episodes = {"Doro": [], "哈基蜂": [], "小黄鸡": []}
    ...
    for f in sorted(WORK_DIR.glob("脚本*_分镜脚本.md")):
        try:
            ...
            has_doro = bool(re.search(r'(Doro|doro|死鱼眼|白色短发)', c))
            has_hachi = bool(re.search(r'(哈基蜂|Hachi|bee|蜜蜂)', c))
            ...
            if has_doro:
                ...
                mode_counts["B_第二角色"] += 1
                char_counts["哈基蜂"] += 1
                char_episodes["哈基蜂"].append(ep_num)
            elif has_chick:
                mode_counts["B_第二角色"] += 1
                char_counts["小黄鸡"] += 1
...
角色出场：Doro={char_counts['Doro']}次 哈基蜂={char_counts['哈基蜂']}次 小黄鸡={char_counts['小黄鸡']}次
